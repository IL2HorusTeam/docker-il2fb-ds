...
This repository contains a definition of a Docker image with the dedicated server of «IL-2 Sturmovik: Forgotten Battles» flight simulator of version `4.13.2` running under 32-bit `wine` `5.0`. See tags for other versions.
...
* Dedicated server of version `4.13.2` installed at `/il2ds` and running via `il2ds` user.
* ENV variables:
...
``` shell
docker run -it --rm -p 0.0.0.0:21000:21000/udp il2horusteam/il2ds:4.13.2-wine5.0
```
...
IL2 FB dedicated server v4.13.2m
...
docker run -it --rm \
  ...
  il2horusteam/il2ds:4.13.2-wine5.0
```
